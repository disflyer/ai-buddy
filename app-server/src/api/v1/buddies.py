...
async def create_buddy(buddy_in: BuddyCreate, db: AsyncSession = Depends(get_db)):
...
async def get_buddy(buddy_id: str, db: AsyncSession = Depends(get_db)):
    service = BuddyService(db)
...
async def update_buddy(buddy_id: str, buddy_in: BuddyUpdate, db: AsyncSession = Depends(get_db)):
    service = BuddyService(db)
...
async def delete_buddy(buddy_id: str, db: AsyncSession = Depends(get_db)):
    service = BuddyService(db)
...
async def bind_buddy(buddy_id: str, child_id: str, db: AsyncSession = Depends(get_db)):
    service = BuddyService(db)
...
async def unbind_buddy(buddy_id: str, db: AsyncSession = Depends(get_db)):
    service = BuddyService(db)
