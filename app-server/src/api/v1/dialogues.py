...
async def create_dialogue(dialogue_in: DialogueCreate, db: AsyncSession = Depends(get_db)):
...
async def get_dialogue(dialogue_id: str, db: AsyncSession = Depends(get_db)):
    service = DialogueService(db)
...
async def update_dialogue(dialogue_id: str, dialogue_in: DialogueUpdate, db: AsyncSession = Depends(get_db)):
    service = DialogueService(db)
...
async def delete_dialogue(dialogue_id: str, db: AsyncSession = Depends(get_db)):
    service = DialogueService(db)
