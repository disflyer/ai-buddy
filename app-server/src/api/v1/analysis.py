...
async def cognitive_analysis(child_id: str = Query(...), db: AsyncSession = Depends(get_db)):
    service = AnalysisService(db)
    return await service.get_cognitive_analysis(child_id)

@router.get("/language", response_model=LanguageAnalysisOut)
async def language_analysis(child_id: str = Query(...), db: AsyncSession = Depends(get_db)):
    service = AnalysisService(db)
    return await service.get_language_analysis(child_id)

@router.get("/health", response_model=HealthAnalysisOut)
async def health_analysis(child_id: str = Query(...), db: AsyncSession = Depends(get_db)):
    service = AnalysisService(db)
